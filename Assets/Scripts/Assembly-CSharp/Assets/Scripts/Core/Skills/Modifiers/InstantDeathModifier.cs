...
{
	public class InstantDeathModifier : Modifier
	{
		public interface Info
		{
			float SuccessRate();

			float HpPercentThreshold();

			bool PlayVfx();
		}

		public class InstantDeathModifierInfo : Info
		{
			private float successRate;

			private float hpPercentThreshold;

			public InstantDeathModifierInfo(float successRate, float hpPercentThreshold)
			{
			}
			...
			public float HpPercentThreshold()
			{
				return 0f;
			}
		...
		public InstantDeathModifier(Info info, Entity target)
		{
		}
